...
const websiteHtmlPath = new URL("../website/index.html", import.meta.url);
...
let websiteHtml = await readFile(websiteHtmlPath, "utf8");
...
if (!cargoVersion) {
  throw new Error(
    "Could not read the application version from src-tauri/Cargo.toml",
  );
}

function assertAligned(expectedTag?: string) {
  const websiteVersion = websiteHtml.match(
    /class="js-version-number">([^<]+)</,
  )?.[1];
  const rootVersion = rootHtml.match(/class="js-version-number">([^<]+)</)?.[1];
  ...
  if (versions.size !== 1) {
    throw new Error(
      `Version mismatch: package.json=${packageJson.version}, tauri.conf.json=${tauriConfig.version}, Cargo.toml=${cargoVersion}, website=${websiteVersion}, root=${rootVersion}`,
    );
  }
  ...
  if (expectedTag && expectedTag !== `v${version}`) {
    throw new Error(
      `Release tag ${expectedTag} must match app version v${version}`,
    );
  }
...
websiteHtml = websiteHtml
  .replace(/(class="js-version-number">)[^<]+/, `$1${requestedVersion}`)
  .replace(/("softwareVersion":")[^"]+/, `$1${requestedVersion}`);
rootHtml = rootHtml
  .replace(/(class="js-version-number">)[^<]+/, `$1${requestedVersion}`)
  .replace(/("softwareVersion":")[^"]+/, `$1${requestedVersion}`);
...
await Promise.all([
  ...
  writeFile(websiteHtmlPath, websiteHtml),
  writeFile(rootHtmlPath, rootHtml),
]);
