...
const assetFor = (assets, matcher) => assets.find((asset) => matcher.test(asset.name));
...
async function hydrateRelease() {
  try {
    ...
    document.querySelectorAll(".js-primary-download").forEach((link) => {
      link.href = platform === "mac" ? "#download" : (preferred?.browser_download_url ?? releasesUrl);
      const small = link.querySelector("small");
      if (small) small.textContent = preferred ? `Download ${platformLabel}` : "View available builds";
    });
