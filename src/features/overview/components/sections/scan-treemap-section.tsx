...
export function ScanTreemapSection({
  summary,
  aside,
}: {
  summary: ScanSummary;
  ...
  return (
    <div className="space-y-4">
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <SectionCard className="overflow-visible border-primary/15 bg-card/90">
          <div className="sticky top-0 z-20 rounded-t-lg border-b border-border bg-card/95 px-4 py-4 backdrop-blur sm:px-5">
            <h2 className="text-sm font-semibold">Storage map</h2>
            <p className="mt-1 text-xs text-muted-foreground">
              {treemap.data
                ? `${treemap.data.totalItems.toLocaleString()} direct items · ${treemap.data.nodes.length} bounded regions · select once, open map folders with double-click or Enter`
                : "Loading space map…"}
            </p>
          </div>
      ...
      <SectionCard className="overflow-hidden border-primary/15 bg-card/90">
        <div className="border-b border-border px-4 py-3 sm:px-5">
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            ...
            <span className="text-[11px] text-muted-foreground">
              {directories.length.toLocaleString()} directories
            </span>
          </div>
        </div>
        {directories.length > 0 ? (
          <div className="divide-y divide-border sm:grid sm:grid-cols-2 sm:divide-y-0 xl:grid-cols-3">
            {directories.map((directory) => {
              return (
                <button
                  key={directory.id}
                  type="button"
                  className="flex w-full items-center justify-between gap-4 border-b border-border px-4 py-3 text-left hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring sm:border-r sm:px-5"
          ...
          <p className="px-5 py-6 text-sm text-muted-foreground">
            This location has no indexed subdirectories. Use the breadcrumb or
